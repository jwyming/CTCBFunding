...
public class SwitchInvestmentBizImpl extends BaseBiz implements
		SwitchInvestmentBiz {

	public void switchInvestment(Trade switchInvestment) {
		//if there is't the frozen fund  account as Source Account,create it
		User user = switchInvestment.getTradeUser();
        FundAccount non_frozenFundAccount = switchInvestment.getAccountBySaccountId().getFundAccount();   
       ...
       if (frozenFundAccount == null) {
           ...
           newAccount.setCreateTime(switchInvestment.getCreateTime());
           ...
           newFundAccount.setIncomingSet(0f);
       ...
       BigDecimal setCount = switchInvestment.getCount().add(switchInvestment.getFee());
		...
		switchInvestment.setAccountBySaccountId(frozenFundAccount.getAccount());
        this.tradeDao.create(switchInvestment);
        // create trade task record in database
        TradeTask tradeTask = new TradeTask();
        tradeTask.setTrade(switchInvestment);
        tradeTask.setCreateTime(switchInvestment.getCreateTime());
        tradeTask.setPlanTime(switchInvestment.getSetTime());
